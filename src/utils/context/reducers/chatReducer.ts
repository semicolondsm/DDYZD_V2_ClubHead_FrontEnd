...
export default function (state: any, action: any) {
    switch (action.type) {
        ...
        case GET_CHAT_LIST_SUCCESS:
            ...
            let temp =
                state.ChatList.data !== null ? [...state.ChatList.data] : [];
        ...
        case READ_MESSAGE:
            ...
            const Rindex = tempR.findIndex(
                (val: any) => val.roomid == action.data
            );
        ...
        case DELETE_LAST_MESSAGE:
            const tempRooms = state.RoomList.data.rooms.map((room: any) => {
                const tempRoom = room;
                if (tempRoom.roomid === action.data.room_id)
                    tempRoom.lastmessage = null;
                return tempRoom;
            });
